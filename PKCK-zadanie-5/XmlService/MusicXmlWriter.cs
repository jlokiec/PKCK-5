...
namespace XmlService
{
    public class MusicXmlWriter
    {
        ...
        {
            var header = new XElement("header", new XElement("authors",
            new XElement[]
            {
                    new XElement("author",
            ...
            var musicians = new XElement("musicians", dataContext.Musicians.Musicians.Select(d => new XElement("musician",
               new XAttribute("musicianId", d.Value.Id),
               new XElement("name", d.Value.Name))).ToArray());

            var genres = new XElement("genres", dataContext.Genres.Genres.Select(d => new XElement("genre",
             new XAttribute("genreId", d.Value.Id),
             new XAttribute("genreName", d.Value.Name))).ToArray());

            var languages = new XElement("languages", dataContext.Languages.Languages.Select(d => new XElement("language",
            new XAttribute("languageId", d.Value.Id), d.Value.Name)).ToArray());

            var distributors = new XElement("distributors", dataContext.Distributors.Distributors.Select(d => new XElement("distributor",
            new XAttribute("distributorId", d.Value.Id), d.Value.Name)).ToArray());

            var catalog = new XElement("catalog", dataContext.Cds.Select(d => new XElement("cd", 
            ...
            var root = new XElement("document", header, musicians, genres, languages, distributors, catalog);
